...
unknown_lc2="<NONE>"
unknown_rc2="<NONE>"
unknown_lc2_label="<NONE_LABEL>"
unknown_lc_label="<NONE_LABEL>"
unknown_rc_label="<NONE_LABEL>"
unknown_rc2_label="<NONE_LABEL>"
unknown_POS="<NONE_POS>"
...
word_dict={}
with open("./data/test/train.txt",'w',encoding='utf-8',) as out:
    with open("./data/test/test.conll", 'r', encoding='utf-8') as f:
        lines = f.readlines()
        # buffer=[]
        ...
        for line in tqdm(lines):
            ...
            if line != ['']:
                ...
                dependency.append(int(line[5]))
                arc_label.append(line[6])
            ...
            if line == ['']:
                ...
                while len(stack) > 1 or buffer != []:
                    if len(stack) == 1:
                        ...
                        if dependency[stack[-1]] == stack[-2]:
                            ...
                            if is_denpency:
                                ...
                                elif rc2[stack[-2]] == -1 and math.fabs(stack[-2] - rc1[stack[-2]]) > math.fabs(
                                        stack[-2] - stack[-1]):
                                    rc2[stack[-2]] = stack[-1]
                    ...
                    for each in feature:
                        if each not in word_dict.keys():
                            word_dict[each]=len(word_dict.keys())
                arc_label = ['<NONE>']  # 因为dependency是从1开始计数，所以最好所有的都加一个初始化数值方便管理
                sentence = ['<ROOT>']
                dependency = ['<NONE>']
                Pos_tag = ["<ROOT>"]
